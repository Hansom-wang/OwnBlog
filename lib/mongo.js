...
mongolass.plugin('addCreatedAt',{
	afterFind:function(results){
		results.forEach(function(item){
			item.created_at=monent(objectIdToTimestamp(item._id)).format('YYYY-MM-DD HH:mm');
		});
		return results;
	},
...
exports.Post.index({author:1,_id:-1}).exec(); //按创建时间倒叙排列
...
exports.Comment.index({postId:1,_id:1}).exec(); //通过文章id获取下面的所有留言，按照创建时间升序
